...
cam_pose = X  # dot(dot(rob, X), inv(obj))
target_pose = dot(dot(rob, X), inv(obj))
...
cam = {
    ...
    'camera_pose': {
        'rotation': cam_pose[0:3, 0:3].tolist(),
        'translation': cam_pose[0:3, -1].tolist(),
    },
    'target_pose': {
        'translation': target_pose[0:3, -1].tolist(),
        'rotation': target_pose[0:3, 0:3].tolist()
    }
}
